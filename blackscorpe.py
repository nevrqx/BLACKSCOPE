...
# -------------------------------
# Менеджер оффсетов и загрузка JSON
# -------------------------------
...
URLS = [
    ...
    "https://raw.githubusercontent.com/a2x/cs2-dumper/main/output/engine2_dll.json",
    "https://raw.githubusercontent.com/a2x/cs2-dumper/main/output/rendersystemdx11_dll.json",
    "https://raw.githubusercontent.com/a2x/cs2-dumper/main/output/worldrenderer_dll.json",
    "https://raw.githubusercontent.com/a2x/cs2-dumper/main/output/animationsystem_dll.json",
    "https://raw.githubusercontent.com/a2x/cs2-dumper/main/output/interfaces.json",
...
INTEREST_KEYS: Dict[str, List[str]] = {
    "dwEntityList": ["dwEntityList"],
    "dwViewMatrix": ["dwViewMatrix", "dwViewMatrix0"],
    "dwLocalPlayerPawn": ["dwLocalPlayerPawn", "dwLocalPlayer"],
    "dwPlantedC4": ["dwPlantedC4"],
    "dwGlowObjectManager": ["dwGlowObjectManager", "dwGlowManager"],
    ...
    # кости/модель (пробуем разные варианты ключей)
    "m_boneArray": [
        "m_pBoneArray", "m_BoneArray", "m_modelState", "m_nBones",
        "m_dwBoneArray", "m_bConstrainBetweenEndpoints"
    ],
...
def _fetch_json(url: str, timeout: int = 15) -> Optional[Dict[str, Any]]:
    try:
        ...
        if r.status_code != 200:
            _log_w(f"Не удалось скачать {url} (HTTP {r.status_code})")
            return None
        return r.json()
    except Exception as e:
        _log_w(f"Ошибка сети/JSON при скачивании {url}: {e}")
        return None

def _deep_find_keys(obj: Any, target_keys: List[str]) -> List[Tuple[str, Union[int, str]]]:
    found: List[Tuple[str, Union[int, str]]] = []
    def rec(node: Any):
...
class OffsetsManager:
    def __init__(self, urls: List[str], preferred_modules: List[str]):
        self.urls = urls
        self.preferred_modules = preferred_modules
        self.data_by_module: Dict[str, Dict[str, Any]] = {}
        self._load_all()

    def _normalize_module_name(self, url: str, root: Dict[str, Any]) -> str:
        if "client_dll.json" in url:
            return "client.dll"
        if "engine2_dll.json" in url:
            return "engine2.dll"
        if "rendersystemdx11_dll.json" in url:
            return "rendersystemdx11.dll"
        if "worldrenderer_dll.json" in url:
            return "worldrenderer.dll"
        if "animationsystem_dll.json" in url:
        ...
        if isinstance(root, dict):
            mod = root.get("module") or root.get("Module") or ""
        return mod if isinstance(mod, str) and mod else f"module_{abs(hash(url)) & 0xffff:x}"
    ...
    def _load_all(self) -> None:
        for u in self.urls:
            ...
            if data is None:
                continue
            name = self._normalize_module_name(u, data)
            self.data_by_module[name] = data
            _log_i(f"Загружен модуль '{name}' из {u}")
        if not self.data_by_module:
            _log_e("Не удалось загрузить ни один JSON с оффсетами")

    def find_offset(self, keys: List[str]) -> Optional[Tuple[str, int]]:
        order: List[str] = []
        if "offsets" in self.data_by_module:
            order.append("offsets")
        for m in self.preferred_modules:
            if m in self.data_by_module and m not in order:
                order.append(m)
        for m in self.data_by_module.keys():
            if m not in order:
                order.append(m)
        for mod in order:
            root = self.data_by_module.get(mod)
            if not root:
                continue
            matches = _deep_find_keys(root, keys)
            for k, v in matches:
                if _is_int_like(v):
                    vi = _to_int(v)
                    if vi is not None:
    ...
    def resolve_interests(self, interest_map: Dict[str, List[str]]) -> Dict[str, Optional[Tuple[str, int]]]:
        out: Dict[str, Optional[Tuple[str, int]]] = {}
        for logical, variants in interest_map.items():
            res = self.find_offset(variants)
            if res is None:
                _log_w(f"Оффсет '{logical}' не найден")
                out[logical] = None
            else:
                m, off = res
                _log_i(f"Оффсет '{logical}': {m} + 0x{off:X}")
                out[logical] = (m, off)
        return out

# -------------------------------
# Резолвер адресов памяти
# -------------------------------
class MemoryResolver:
    def __init__(self, pm: pymem.Pymem, resolved: Dict[str, Optional[Tuple[str, int]]], module_bases: Dict[str, int]):
        self.pm = pm
        self.resolved = resolved
        self.module_bases = module_bases  # {'client.dll': base, ...}

    def get_module_base(self, module_name: str) -> Optional[int]:
        return self.module_bases.get(module_name)

    def get_abs(self, logical: str) -> Optional[int]:
        entry = self.resolved.get(logical)
        if entry is None:
            _log_w(f"Оффсет '{logical}' отсутствует")
            return None
        mod, off = entry
        if mod == "offsets":
            # Пробуем клиент, затем fallback
            for m in PREFERRED_MODULES:
                base = self.get_module_base(m)
                if base:
                    return base + off
            _log_w(f"Не удалось подобрать базу для глобального оффсета '{logical}'")
            return None
        base = self.get_module_base(mod)
        if base is None:
            _log_w(f"Модуль '{mod}' не загружен (для '{logical}')")
            return None
        return base + off
...
class ESPWindow(QtWidgets.QWidget):
    def __init__(self, settings, shutdown_event=None):
        ...
        # Менеджер оффсетов: загрузка всех JSON + fallback
        try:
            self._offman = OffsetsManager(URLS, PREFERRED_MODULES)
            self._resolved = self._offman.resolve_interests(INTEREST_KEYS)
            print("Оффсеты успешно загружены")
        except Exception as e:
            print(f"Ошибка при загрузке оффсетов: {e}")
            raise

        # Подключаемся к игре и кэшируем базы модулей
        try:
            self.pm = pymem.Pymem("cs2.exe")
            self._module_bases: Dict[str, int] = {}
            # Пытаемся получить базы для всех интересующих модулей
            for mod_name in set(PREFERRED_MODULES + list(self._offman.data_by_module.keys())):
                if mod_name in ("offsets", "interfaces"):
                    continue
                try:
                    m = pymem.process.module_from_name(self.pm.process_handle, mod_name)
                    if m:
                        self._module_bases[mod_name] = m.lpBaseOfDll
                        if mod_name == "client.dll":
                            self.client = m.lpBaseOfDll  # для обратной совместимости
                except Exception:
                    pass
            # Резолвер абсолютных адресов
            self._resolver = MemoryResolver(self.pm, self._resolved, self._module_bases)
            print("Успешное подключение к игре и кэширование модулей")
        except Exception as e:
            print(f"Ошибка при подключении к игре: {e}")
            raise

        # JSON структуры классов из client_dll.json (для нетваров/полей)
        self.client_dll = self._offman.data_by_module.get("client.dll", {})
    ...
    def _run_assist_logic(self):
        try:
            ...
            # Получаем матрицу и локала (через резолвер абсолютных адресов)
            a_dwEntityList = self._resolver.get_abs('dwEntityList')
            a_dwLocalPlayerPawn = self._resolver.get_abs('dwLocalPlayerPawn')
            a_dwViewMatrix = self._resolver.get_abs('dwViewMatrix')
            if not a_dwEntityList or not a_dwLocalPlayerPawn or not a_dwViewMatrix:
                return
            ...
            view_matrix = [self.pm.read_float(a_dwViewMatrix + i * 4) for i in range(16)]
            local_player_pawn_addr = self.pm.read_longlong(a_dwLocalPlayerPawn)
            try:
                local_team = self.pm.read_int(local_player_pawn_addr + m_iTeamNum)
            except Exception:
                return

            entity_list = self.pm.read_longlong(a_dwEntityList)
    ...
    def update_scene(self):
        ...
        try:
            ...
            esp(self.scene, self.pm, self._resolver, self.client_dll, self.window_width, self.window_height, self.settings)
            ...
            for mod_name in set(PREFERRED_MODULES + list(self._offman.data_by_module.keys())):
                ...
                try:
                    ...
                    if m:
                        self._module_bases[mod_name] = m.lpBaseOfDll
                        if mod_name == "client.dll":
                            self.client = m.lpBaseOfDll
                except Exception:
                    pass
        except Exception as e:
            print(f"heal: refresh module bases failed: {e}")
            try:
                self.pm = pymem.Pymem("cs2.exe")
            except Exception as e2:
                print(f"heal: reopen Pymem failed: {e2}")
        try:
            # Перечитываем оффсеты и резолвер
            self._offman = OffsetsManager(URLS, PREFERRED_MODULES)
            self._resolved = self._offman.resolve_interests(INTEREST_KEYS)
            self._resolver = MemoryResolver(self.pm, self._resolved, self._module_bases)
            self.client_dll = self._offman.data_by_module.get("client.dll", {})
        except Exception as e:
            print(f"heal: refresh offsets/resolver failed: {e}")
        # сбрасываем счётчик пустых кадров
        self._empty_frames = 0

def esp(scene, pm, resolver: MemoryResolver, client_dll, window_width, window_height, settings):
    if settings['esp_rendering'] == 0:
        return

    a_dwEntityList = resolver.get_abs('dwEntityList')
    a_dwLocalPlayerPawn = resolver.get_abs('dwLocalPlayerPawn')
    a_dwViewMatrix = resolver.get_abs('dwViewMatrix')
    a_dwPlantedC4 = resolver.get_abs('dwPlantedC4')
    if not a_dwEntityList or not a_dwLocalPlayerPawn or not a_dwViewMatrix:
        return
    ...
    view_matrix = [pm.read_float(a_dwViewMatrix + i * 4) for i in range(16)]

    local_player_pawn_addr = pm.read_longlong(a_dwLocalPlayerPawn)
    try:
    ...
    entity_list = pm.read_longlong(a_dwEntityList)
    entity_ptr = pm.read_longlong(entity_list + 0x10)

    def bombisplant():
        global BombPlantedTime
        if not a_dwPlantedC4:
            return False
        bombisplant = pm.read_bool(a_dwPlantedC4 - 0x8)
        if bombisplant:
    ...
    def getC4BaseClass():
        if not a_dwPlantedC4:
            return 0
        plantedc4 = pm.read_longlong(a_dwPlantedC4)
